...
pcall(function()
    ...
    if isNew then
        writefile("voidware_linoria/themes/default.txt", "Default")
        local suc = pcall(function()
...
pcall(function()
    local themeContent = [==[
{"MainColor":"090909","FontFace":"Jura","AccentColor":"7d55ff","OutlineColor":"141414","BackgroundColor":"080808","FontColor":"ffffff"}
]==]
    
    local themesFolder = "voidware_linoria/themes"
    
    if isfolder(themesFolder) then
        local themeFilePath = themesFolder .. "/123.json"
        
        writefile(themeFilePath, themeContent)
        print("123.json create")
        
        local settingsFolder = "voidware_linoria/themes"
        if isfolder(settingsFolder) then
            local autoloadPath = settingsFolder .. "/default.txt"
            
            writefile(autoloadPath, "123")
            print("theme set default 123")
        else
            warn("dont found " .. settingsFolder)
        end
    else
        warn("dont found fold " .. themesFolder)
    end
end)
...
--// Library \\--
local repo = "https://raw.githubusercontent.com/NightForRoblox/"..tostring(targetlib).."/main/"
...
local Window = Library:CreateWindow({
	Title = "ANGELWINGS | Ink",
    Footer = "by AngelWings Team",
	Center = true,
	AutoShow = true,
	Resizable = true,
    Icon = 107892019054573,
...
local Tabs = {
    ...
    Other = Window:AddTab("Other", "home", "Other features"),
    Misc = Window:AddTab("Misc", "home", "Misc features"),
    Visuals = Window:AddTab("Visuals", "home", "Visual features"),
...
local Existing = CoreGui:FindFirstChild("AlertSound")

local AlertSound = Existing or Instance.new("Sound")
AlertSound.SoundId = "rbxassetid://12222030"
AlertSound.Volume = 1
AlertSound.Name = "AlertSound"
AlertSound.Parent = CoreGui
function Script.Functions.Alert(message: string, time_obj: number)
    Library:Notify(message, time_obj or 5)
    if AlertSound.IsLoaded then
        AlertSound:Play()
    else
        task.spawn(function()
            AlertSound.Loaded:Wait()
            AlertSound:Play()
        end)
    end
end
...
Script.Functions.PullRope = function(perfect)
    local args = {
	buffer.fromstring("\196\192\193\194\193\193\193\193\193\1931\254\196\192\193\197\201\193\193\193\136\137\160\181\164\152\174\180\192"),
	{}
}
...
function Script.Functions.RevealGlassBridge()
    ...
    Effects.AnnouncementTween({
        ...
        FasterTween = true,
        DisplayTime = 10,
        AnnouncementDisplayText = "[AngelWings]: Safe tiles are green, breakable tiles are red!"
    })
end

function Script.Functions.AntiFallGlassBridge()
    local glassHolder = workspace:FindFirstChild("GlassBridge") and workspace.GlassBridge:FindFirstChild("GlassHolder")
    if not glassHolder then
    ...
    for _, tilePair in pairs(glassHolder:GetChildren()) do
        for _, tileModel in pairs(tilePair:GetChildren()) do
            if tileModel:IsA("Model") and tileModel.PrimaryPart then
                ...
                if primaryPart:GetAttribute("exploitingisevil") == true then
                    ...
                    if glassPart and glassPart:IsA("BasePart") then
                        local arbuz = glassPart:Clone()
                        arbuz.Name = "arbuz"
                        arbuz.Transparency = 1
                        arbuz.CanCollide = true
                        arbuz.CanQuery = true
                        arbuz.CastShadow = false
                        arbuz.Parent = tileModel
    ...
    Script.Functions.Alert(string.format("Anti Fall: Removed all broken glass", removedCount, createdCount), 5)
end
...
Script.Functions.OnLoad = function()
    ...
    Script.Functions.EffectsNotification("AngelWings - Ink Game loaded!", 5)
end
...
Script.Functions.JoinDiscordServer = function()
    local sInvite = "https://discord.com/invite/jUuYn6gRjk"
    ...
    pcall(function()
        setclipboard("discord.gg/jUuYn6gRjk")
    end)
    game:GetService("StarterGui"):SetCore("SendNotification", {
        Title = "AngelWings Discord - discord.gg/jUuYn6gRjk",
        Text = "Copied to clipboard (discord.gg/jUuYn6gRjk)",
        Duration = 10,
    })
end
...
local SelfGroupBox = Tabs.Visuals:AddRightGroupbox("Self", "user")   
...
local GreenLightRedLightGroup = Tabs.Main:AddLeftGroupbox("Red Light / Green Light", "traffic-light") do
    ...
    Toggles.RedLightGodmode:OnChanged(function(enabled)
        if enabled then
            ...
            Script.Functions.Alert("DETECTED Red Light Green Light Godmode", 3)
        else
            ...
            Script.Functions.Alert("Red Light Green Light Godmode Disabled DETECTED", 3)
        end
    end)
    ]]
...
Toggles.ImmuneDalgonaGame:OnChanged(function(call)
    if call then
        ...
        Script.Functions.Alert("Your cookie will be breaked from now on!", 3)
    else
        print("detected")
...
local MingleGroup = Tabs.Main:AddLeftGroupbox("Mingle", "users") do
    MingleGroup:AddToggle("AutoMingleQTE", {
        Text = "Auto Mingle QTE",
        Default = false
    })
    
    MingleGroup:AddToggle("RageMingleQTE", {
        Text = "Rage Mingle QTE",
        Default = false
    })
    
    local function runMingleLoop(cooldown, isRage)
        local threadName = isRage and "RageMingleQTEThread" or "AutoMingleQTEThread"
        local activeFlag = isRage and "RageMingleQTEActive" or "AutoMingleQTEActive"
        
        Script.Temp[activeFlag] = true
        
        Script.Temp[threadName] = task.spawn(function()
            while Script.Temp[activeFlag] do
    ...
    local function stopMingleLoop(isRage)
        local threadName = isRage and "RageMingleQTEThread" or "AutoMingleQTEThread"
        local activeFlag = isRage and "RageMingleQTEActive" or "AutoMingleQTEActive"
        
        Script.Temp[activeFlag] = false
        
        if Script.Temp[threadName] then
            pcall(function()
                task.cancel(Script.Temp[threadName])
            end)
            Script.Temp[threadName] = nil
        end
    end
    
    Toggles.AutoMingleQTE:OnChanged(function(enabled)
        if enabled and Toggles.RageMingleQTE.Value then
            Toggles.RageMingleQTE:SetValue(false)
        end
        
        if enabled then
            Script.Functions.Alert("Auto Mingle Enabled", 3)
            runMingleLoop(0.1, false)
        else
            Script.Functions.Alert("Auto Mingle Disabled", 3)
            stopMingleLoop(false)
        end
    end)
    
    Toggles.RageMingleQTE:OnChanged(function(enabled)
        if enabled and Toggles.AutoMingleQTE.Value then
            Toggles.AutoMingleQTE:SetValue(false)
        end
        
        if enabled then
            Script.Functions.Alert("RAGE Mingle Enabled", 3)
            runMingleLoop(0, true)
        else
            Script.Functions.Alert("RAGE Mingle Disabled", 3)
            stopMingleLoop(true)
        end
    end)
end
...
local GlassBridgeGroup = Tabs.Main:AddLeftGroupbox("Glass Bridge", "bridge") do
    ...
    GlassBridgeGroup:AddButton("Anti Fall Glass Bridge", function()
    if not workspace:FindFirstChild("GlassBridge") then
        Script.Functions.Alert("Game not running")
        return
    end
    Script.Functions.AntiFallGlassBridge()
end)
end

local SkySquidGameGroup = Tabs.Main:AddLeftGroupbox("Sky Squid Game", "cloud") do
    SkySquidGameGroup:AddToggle("AutoStamina", {
        Text = "Auto Stamina",
        Default = false
    })
    
    Toggles.AutoStamina:OnChanged(function(enabled)
        if enabled then
            Script.Functions.Alert("Auto Stamina Enabled", 3)
            
            Script.Temp.AutoStaminaLoop = task.spawn(function()
                while Toggles.AutoStamina.Value and not Library.Unloaded do
                    local liveFolder = workspace:FindFirstChild("Live")
                    if liveFolder then
                        local playerModel = liveFolder:FindFirstChild(lplr.Name)
                        
                        if playerModel then
                            local stamina = playerModel:FindFirstChild("CurrentStaminaNumberValue")
            ...
            Script.Functions.Alert("Auto Stamina Disabled", 3)
            
            if Script.Temp.AutoStaminaLoop then
                pcall(function()
                    task.cancel(Script.Temp.AutoStaminaLoop)
                end)
                Script.Temp.AutoStaminaLoop = nil
            end
        end
    end)
end
...
local InformationGroup = Tabs.Main:AddRightGroupbox("Information", "info") do
    InformationGroup:AddLabel("Welcome to AngelWings!")
    InformationGroup:AddLabel("This is - VoidWare Rework.")
    InformationGroup:AddLabel("Some functions maybe detected.")
    InformationGroup:AddLabel("If you find any such function,")
    InformationGroup:AddLabel("please report it on our Discord.")
...
local currentPentathlonGUID = nil

local function setupPentathlonHook()
    local pentathlon = require(game:GetService("ReplicatedStorage").Modules.Games.PentathlonClient)
    local oldAddGame = pentathlon.AddActiveGame
    pentathlon.AddActiveGame = function(gameData)
        if gameData and gameData.GUID then
            currentPentathlonGUID = gameData.GUID
        end
        return oldAddGame(gameData)
    end
end

task.spawn(function()
    pcall(setupPentathlonHook)
end)

local function getRemainingTime()
    local player = game:GetService("Players").LocalPlayer
    local gonggiUI = player.PlayerGui:FindFirstChild("OtherUIHolder") 
        and player.PlayerGui.OtherUIHolder:FindFirstChild("Gonggi")
    
    if not gonggiUI then
        return nil
    end
    
    local timeLabel = gonggiUI:FindFirstChild("TmeRemaining")
    ...
    local timeValue = tonumber(number)
    
    return timeValue
end

local PentathlonGroup = Tabs.Main:AddRightGroupbox("Pentathlon") do
    PentathlonGroup:AddButton("Win Ddakji (1 game)", function()
        if not currentPentathlonGUID then
            Library:Notify("No active Pentathlon game detected", 3)
            return
        end
        
        local args = {
            currentPentathlonGUID,
            "Thrown",
            {
                Power = 0.9438757389662948,
                Position = vector.create(-147.4780731201172, 6001.07568359375, -20.425966262817383)
            }
        }
        
        game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("PentathlonRemote"):FireServer(unpack(args))
        Library:Notify("ready game 1! or not try again", 2)
    end)
    
    PentathlonGroup:AddButton("Win Biseokchigi (2 game)", function()
        if not currentPentathlonGUID then
            Library:Notify("No active Pentathlon game detected", 3)
            return
        end
        ...
        local args = {
            ...
            "Thrown",
            {
                Direction = vector.create(-0.8497589230537415, -0.19904698431491852, 0.48814964294433594),
                ThrowPower = "Perfect",
                Origin = vector.create(-176.9093475341797, 6005.49560546875, -20.59357261657715)
            }
        }
        
        game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("PentathlonRemote"):FireServer(unpack(args))
        Library:Notify("ready game 2!", 2)
    end)
    
    PentathlonGroup:AddButton("Win Gonggi (3 game)", function()
        if not currentPentathlonGUID then
            Library:Notify("No active Pentathlon game detected", 3)
            return
        end
        
        Library:Notify("starting 3 game sequence...", 3)
        
        local pieces = {
            { name = "BCircle", label = "BCircle" },
            { name = "YTriangle", label = "YTriangle" },
            { name = "Square", label = "Square" },
            { name = "GTriangle", label = "GTriangle" },
            { name = "RCircle", label = "RCircle" }
        }
        ...
        task.spawn(function()
            for i, piece in ipairs(pieces) do
                ...
                if i < #pieces then
                    ...
                    if remainingTime and remainingTime > 0 then
                        local waitTime = remainingTime + 3
                        task.wait(waitTime)
                    else
                        task.wait(5)
                    end
            ...
            Library:Notify("ready game 3!", 3)
        end)
    end)
    
    PentathlonGroup:AddButton("Win Spinning Top (4 game)", function()
        if not currentPentathlonGUID then
            Library:Notify("No active Pentathlon game detected", 3)
            return
        end
        
        Library:Notify("starting 4 game sequence...", 2)
        ...
        task.spawn(function()
            ...
            game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("PentathlonRemote"):FireServer(unpack(args1))
            Library:Notify("Tied sent, waiting 3s...", 2)
            ...
            game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("PentathlonRemote"):FireServer(unpack(args2))
            Library:Notify("ready game 4!", 2)
        end)
    end)
    
    PentathlonGroup:AddButton("Win Jegi (5 game)", function()
        if not currentPentathlonGUID then
            Library:Notify("No active Pentathlon game detected", 3)
            return
        end
        ...
        task.spawn(function()
            for i = 1, 6 do
                ...
                game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("PentathlonRemote"):FireServer(unpack(args))
                Library:Notify("Kick " .. i .. "/6 sent", 1)
...
local RebelGroup = Tabs.Main:AddRightGroupbox("Rebel", "sword") do
    ...
    RebelGroup:AddSlider("HitboxSizeSlider", {
        Text = "Hitbox Size",
        Default = 4,
        Min = 2,
        Max = 15,
            ...
            task.spawn(function()
                repeat
                    ...
                    local targetSize = Vector3.new(
                        Options.HitboxSizeSlider.Value,
                        Options.HitboxSizeSlider.Value,
                        Options.HitboxSizeSlider.Value
                    )
    ...
    Options.HitboxSizeSlider:OnChanged(function(val)
        if not Toggles.ExpandGuardHitbox.Value then return end
        
        local targetSize = Vector3.new(val, val, val)
    ...
    Script.Functions.Alert("Temporarily unwork", 3)
    --[[
...
local GunMods = Tabs.Main:AddRightGroupbox("Gun Mods", "sword") do
    GunMods:AddToggle("RapidFire", {
        Text = "RapidFire (Instant)",
        Default = false
    })
    
    GunMods:AddToggle("CustomRapidFire", {
        Text = "Custom RapidFire",
        Default = false
    })
    
    GunMods:AddToggle("CustomMultiShot", {
        Text = "Custom Bullets Per Fire",
        Default = false
    })
    ...
    GunMods:AddToggle("CustomSpread", {
        Text = "Custom Spread",
        Default = false
    })
    ...
    GunMods:AddToggle("CustomRecoil", {
        Text = "Custom Recoil",
        Default = false
    })
    ...
    GunMods:AddSlider("CustomFireRate", {
    ...
    GunMods:AddSlider("CustomBulletsPerFire", {
        Text = "Bullets Per Fire",
        Default = 1,
        Min = 1,
        Max = 100,
        Rounding = 0
    ...
    GunMods:AddSlider("CustomSpreadValue", {
        Text = "Custom Spread Value",
        Default = 0,
        Min = 0,
        Max = 100,
        Rounding = 1
    ...
    GunMods:AddSlider("CustomRecoilValue", {
        Text = "Custom Recoil Value",
        Default = 0,
        Min = 0,
        Max = 10,
        Rounding = 2
    ...
    Script.Temp.OriginalFireRates = {}
    Script.Temp.OriginalBulletsPerFire = {}
    Script.Temp.OriginalSpreads = {}
    Script.Temp.OriginalRecoils = {}
    Script.Temp.CreatedRecoilFolders = {}
    Script.Temp.OriginalBullets = {}
    Script.Temp.InfAmmoEnabled = false
    
    local function getAvailableGuns()
        local gunsFolder = game:GetService("ReplicatedStorage"):FindFirstChild("Weapons") 
            and game:GetService("ReplicatedStorage").Weapons:FindFirstChild("Guns")
        
        if not gunsFolder then return {} end
        
        local availableGuns = {}
        for _, gun in pairs(gunsFolder:GetChildren()) do
            if gun:IsA("Model") or gun:IsA("Folder") or gun:IsA("Configuration") then
                table.insert(availableGuns, gun.Name)
            end
        end
    ...
    local function playerHasGun(gunName)
        if lplr.Character then
    ...
    local function setFireRate(enabled, customValue)
        local gunsFolder = game:GetService("ReplicatedStorage"):FindFirstChild("Weapons") 
            and game:GetService("ReplicatedStorage").Weapons:FindFirstChild("Guns")
        
        if not gunsFolder then return 0 end
        
        local modifiedCount = 0
        local targetValue = customValue ~= nil and customValue or 0
        
        for _, gun in pairs(gunsFolder:GetChildren()) do
            local fireRateCD = gun:FindFirstChild("FireRateCD")
            
            if fireRateCD and fireRateCD:IsA("NumberValue") then
                if enabled then
                    if Script.Temp.OriginalFireRates[gun.Name] == nil then
                        Script.Temp.OriginalFireRates[gun.Name] = fireRateCD.Value
                    end
                    fireRateCD.Value = targetValue
                    modifiedCount = modifiedCount + 1
                else
                    if Script.Temp.OriginalFireRates[gun.Name] ~= nil then
                        fireRateCD.Value = Script.Temp.OriginalFireRates[gun.Name]
                        Script.Temp.OriginalFireRates[gun.Name] = nil
                        modifiedCount = modifiedCount + 1
                    end
                end
            end
    ...
    local function setBulletsPerFire(enabled, customValue)
        local gunsFolder = game:GetService("ReplicatedStorage"):FindFirstChild("Weapons") 
            and game:GetService("ReplicatedStorage").Weapons:FindFirstChild("Guns")
        ...
        local modifiedCount = 0
        local targetValue = customValue ~= nil and customValue or 1
        
        for _, gun in pairs(gunsFolder:GetChildren()) do
            local bulletsPerFire = gun:FindFirstChild("BulletsPerFire")
            
            if bulletsPerFire and (bulletsPerFire:IsA("IntValue") or bulletsPerFire:IsA("NumberValue")) then
                if enabled then
                    if Script.Temp.OriginalBulletsPerFire[gun.Name] == nil then
                        Script.Temp.OriginalBulletsPerFire[gun.Name] = bulletsPerFire.Value
                    end
                    bulletsPerFire.Value = targetValue
                    modifiedCount = modifiedCount + 1
                else
                    if Script.Temp.OriginalBulletsPerFire[gun.Name] ~= nil then
                        bulletsPerFire.Value = Script.Temp.OriginalBulletsPerFire[gun.Name]
                        Script.Temp.OriginalBulletsPerFire[gun.Name] = nil
                        modifiedCount = modifiedCount + 1
                    end
                end
            end
    ...
    local function setSpread(enabled, customValue)
        local gunsFolder = game:GetService("ReplicatedStorage"):FindFirstChild("Weapons") 
            and game:GetService("ReplicatedStorage").Weapons:FindFirstChild("Guns")
        ...
        for _, gun in pairs(gunsFolder:GetChildren()) do
            ...
            if spread and (spread:IsA("IntValue") or spread:IsA("NumberValue")) then
                if enabled then
                    if Script.Temp.OriginalSpreads[gun.Name] == nil then
                        Script.Temp.OriginalSpreads[gun.Name] = spread.Value
                    end
                    spread.Value = targetValue
                    modifiedCount = modifiedCount + 1
                else
                    if Script.Temp.OriginalSpreads[gun.Name] ~= nil then
                        spread.Value = Script.Temp.OriginalSpreads[gun.Name]
                        Script.Temp.OriginalSpreads[gun.Name] = nil
                        modifiedCount = modifiedCount + 1
                    end
                end
            end
    ...
    local function setRecoil(enabled, customValue)
        local gunsFolder = game:GetService("ReplicatedStorage"):FindFirstChild("Weapons") 
            and game:GetService("ReplicatedStorage").Weapons:FindFirstChild("Guns")
        ...
        for _, gun in pairs(gunsFolder:GetChildren()) do
            local recoilFolder = gun:FindFirstChild("Recoil")
            
            if not recoilFolder then
                recoilFolder = Instance.new("Folder")
                recoilFolder.Name = "Recoil"
                recoilFolder.Parent = gun
                Script.Temp.CreatedRecoilFolders[gun.Name] = true
            end
            
            if enabled then
                local currentRecoil = recoilFolder:GetAttribute("Recoil") or 0
                if Script.Temp.OriginalRecoils[gun.Name] == nil then
                    Script.Temp.OriginalRecoils[gun.Name] = currentRecoil
                end
                recoilFolder:SetAttribute("Recoil", targetValue)
                modifiedCount = modifiedCount + 1
            else
                if Script.Temp.OriginalRecoils[gun.Name] ~= nil then
                    recoilFolder:SetAttribute("Recoil", Script.Temp.OriginalRecoils[gun.Name])
                    Script.Temp.OriginalRecoils[gun.Name] = nil
                    modifiedCount = modifiedCount + 1
                end
                
                if Script.Temp.CreatedRecoilFolders[gun.Name] then
                    recoilFolder:Destroy()
                    Script.Temp.CreatedRecoilFolders[gun.Name] = nil
                end
            end
        end
    ...
    local function setInfAmmo(enabled)
        ...
        local availableGuns = getAvailableGuns()
        
        for _, gunName in pairs(availableGuns) do
            local playerGun, location = playerHasGun(gunName)
            
            if playerGun then
                local info = playerGun:FindFirstChild("Info")
                if info then
                    local bullets = info:FindFirstChild("Bullets")
                    if bullets and (bullets:IsA("IntValue") or bullets:IsA("NumberValue")) then
                        if enabled then
                            if Script.Temp.OriginalBullets[gunName .. "_Info"] == nil then
                                Script.Temp.OriginalBullets[gunName .. "_Info"] = bullets.Value
                                print("[InfAmmo] Saved Info.Bullets for " .. gunName .. " (" .. location .. "): " .. bullets.Value)
                            end
                            bullets.Value = 999999
                            modifiedCount = modifiedCount + 1
                        else
                            if Script.Temp.OriginalBullets[gunName .. "_Info"] ~= nil then
                                bullets.Value = Script.Temp.OriginalBullets[gunName .. "_Info"]
                                print("[InfAmmo] Restored Info.Bullets for " .. gunName .. ": " .. bullets.Value)
                                Script.Temp.OriginalBullets[gunName .. "_Info"] = nil
                                modifiedCount = modifiedCount + 1
                            end
                        end
                    end
                ...
                local infoClient = playerGun:FindFirstChild("InfoClient")
                if infoClient then
                    local bulletsClient = infoClient:FindFirstChild("Bullets")
                    if bulletsClient and (bulletsClient:IsA("IntValue") or bulletsClient:IsA("NumberValue")) then
                        if enabled then
                            if Script.Temp.OriginalBullets[gunName .. "_InfoClient"] == nil then
                                Script.Temp.OriginalBullets[gunName .. "_InfoClient"] = bulletsClient.Value
                                print("[InfAmmo] Saved InfoClient.Bullets for " .. gunName .. " (" .. location .. "): " .. bulletsClient.Value)
                            end
                            bulletsClient.Value = 999999
                            modifiedCount = modifiedCount + 1
                        else
                            if Script.Temp.OriginalBullets[gunName .. "_InfoClient"] ~= nil then
                                bulletsClient.Value = Script.Temp.OriginalBullets[gunName .. "_InfoClient"]
                                print("[InfAmmo] Restored InfoClient.Bullets for " .. gunName .. ": " .. bulletsClient.Value)
                                Script.Temp.OriginalBullets[gunName .. "_InfoClient"] = nil
                                modifiedCount = modifiedCount + 1
                            end
                        end
                    end
    ...
    local function infAmmoLoop()
        while Script.Temp.InfAmmoEnabled and not Library.Unloaded do
            setInfAmmo(true)
            task.wait(0.3)
        end
    end
    
    Toggles.RapidFire:OnChanged(function(enabled)
        if enabled then
            if Toggles.CustomRapidFire.Value then Toggles.CustomRapidFire:SetValue(false) end
            
            local count = setFireRate(true, 0)
            Script.Functions.Alert("RapidFire: " .. count .. " guns", 3)
            
            Script.Temp.RapidFireLoop = task.spawn(function()
                while Toggles.RapidFire.Value and not Library.Unloaded do
                    setFireRate(true, 0)
                    task.wait(0.5)
                end
            end)
        else
            local count = setFireRate(false)
            Script.Functions.Alert("RapidFire Disabled: " .. count .. " restored", 3)
            
            if Script.Temp.RapidFireLoop then
                task.cancel(Script.Temp.RapidFireLoop)
                Script.Temp.RapidFireLoop = nil
            end
        end
    end)
    
    Toggles.CustomRapidFire:OnChanged(function(enabled)
        if enabled then
            if Toggles.RapidFire.Value then Toggles.RapidFire:SetValue(false) end
            
            local val = Options.CustomFireRate.Value
            local count = setFireRate(true, val)
            Script.Functions.Alert("Custom FireRate: " .. val .. " | " .. count .. " guns", 3)
            
            Script.Temp.CustomRapidFireLoop = task.spawn(function()
                while Toggles.CustomRapidFire.Value and not Library.Unloaded do
                    setFireRate(true, Options.CustomFireRate.Value)
                    task.wait(0.3)
                end
            end)
        else
            local count = setFireRate(false)
            Script.Functions.Alert("Custom RapidFire Disabled: " .. count .. " restored", 3)
            
            if Script.Temp.CustomRapidFireLoop then
                task.cancel(Script.Temp.CustomRapidFireLoop)
                Script.Temp.CustomRapidFireLoop = nil
            end
        end
    end)
    
    Toggles.CustomMultiShot:OnChanged(function(enabled)
        if enabled then
            local val = Options.CustomBulletsPerFire.Value
            local count = setBulletsPerFire(true, val)
            Script.Functions.Alert("Custom MultiShot: " .. val .. " bullets | " .. count .. " guns", 3)
            
            Script.Temp.CustomMultiShotLoop = task.spawn(function()
                while Toggles.CustomMultiShot.Value and not Library.Unloaded do
                    setBulletsPerFire(true, Options.CustomBulletsPerFire.Value)
                    task.wait(0.3)
                end
            end)
        else
            local count = setBulletsPerFire(false)
            Script.Functions.Alert("Custom MultiShot Disabled: " .. count .. " restored", 3)
            
            if Script.Temp.CustomMultiShotLoop then
                task.cancel(Script.Temp.CustomMultiShotLoop)
                Script.Temp.CustomMultiShotLoop = nil
            end
        end
    end)
    
    Toggles.NoSpread:OnChanged(function(enabled)
        if enabled then
            if Toggles.CustomSpread.Value then Toggles.CustomSpread:SetValue(false) end
            
            local count = setSpread(true, 0)
            Script.Functions.Alert("No Spread: " .. count .. " guns", 3)
            
            Script.Temp.NoSpreadLoop = task.spawn(function()
                while Toggles.NoSpread.Value and not Library.Unloaded do
                    setSpread(true, 0)
                    task.wait(0.5)
                end
            end)
        else
            local count = setSpread(false)
            Script.Functions.Alert("No Spread Disabled: " .. count .. " restored", 3)
            
            if Script.Temp.NoSpreadLoop then
                task.cancel(Script.Temp.NoSpreadLoop)
                Script.Temp.NoSpreadLoop = nil
            end
        end
    end)
    
    Toggles.CustomSpread:OnChanged(function(enabled)
        if enabled then
            if Toggles.NoSpread.Value then Toggles.NoSpread:SetValue(false) end
            
            local val = Options.CustomSpreadValue.Value
            local count = setSpread(true, val)
            Script.Functions.Alert("Custom Spread: " .. val .. " | " .. count .. " guns", 3)
            
            Script.Temp.CustomSpreadLoop = task.spawn(function()
                while Toggles.CustomSpread.Value and not Library.Unloaded do
                    setSpread(true, Options.CustomSpreadValue.Value)
                    task.wait(0.3)
                end
            end)
        else
            local count = setSpread(false)
            Script.Functions.Alert("Custom Spread Disabled: " .. count .. " restored", 3)
            
            if Script.Temp.CustomSpreadLoop then
                task.cancel(Script.Temp.CustomSpreadLoop)
                Script.Temp.CustomSpreadLoop = nil
            end
        end
    end)
    
    Toggles.NoRecoil:OnChanged(function(enabled)
        if enabled then
            if Toggles.CustomRecoil.Value then Toggles.CustomRecoil:SetValue(false) end
            
            local count = setRecoil(true, 0)
            Script.Functions.Alert("No Recoil: " .. count .. " guns", 3)
            
            Script.Temp.NoRecoilLoop = task.spawn(function()
                while Toggles.NoRecoil.Value and not Library.Unloaded do
                    setRecoil(true, 0)
                    task.wait(0.5)
                end
            end)
        else
            local count = setRecoil(false)
            Script.Functions.Alert("No Recoil Disabled: " .. count .. " restored", 3)
            
            if Script.Temp.NoRecoilLoop then
                task.cancel(Script.Temp.NoRecoilLoop)
                Script.Temp.NoRecoilLoop = nil
            end
        end
    end)
    
    Toggles.CustomRecoil:OnChanged(function(enabled)
        if enabled then
            if Toggles.NoRecoil.Value then Toggles.NoRecoil:SetValue(false) end
            
            local val = Options.CustomRecoilValue.Value
            local count = setRecoil(true, val)
            Script.Functions.Alert("Custom Recoil: " .. val .. " | " .. count .. " guns", 3)
            
            Script.Temp.CustomRecoilLoop = task.spawn(function()
                while Toggles.CustomRecoil.Value and not Library.Unloaded do
                    setRecoil(true, Options.CustomRecoilValue.Value)
                    task.wait(0.3)
                end
            end)
        else
            local count = setRecoil(false)
            Script.Functions.Alert("Custom Recoil Disabled: " .. count .. " restored", 3)
            
            if Script.Temp.CustomRecoilLoop then
                task.cancel(Script.Temp.CustomRecoilLoop)
                Script.Temp.CustomRecoilLoop = nil
            end
        end
    end)
    
    Toggles.InfAmmo:OnChanged(function(enabled)
        Script.Temp.InfAmmoEnabled = enabled
        
        if enabled then
            local count = setInfAmmo(true)
            if count > 0 then
                Script.Functions.Alert("Infinite Ammo: " .. count .. " guns equipped", 3)
            else
                Script.Functions.Alert("Infinite Ammo: No guns found! Equip weapon first.", 3)
            end
            
            Script.Temp.InfAmmoLoop = task.spawn(infAmmoLoop)
            
            Script.Temp.InfAmmoToolAdded = lplr.Character.ChildAdded:Connect(function(child)
                if child:IsA("Tool") and Script.Temp.InfAmmoEnabled then
                    task.wait(0.2)
                    local newCount = setInfAmmo(true)
                    if newCount > 0 then
                        Script.Functions.Alert("Inf Ammo applied to: " .. child.Name, 2)
                    end
                end
            end)
            
            Script.Temp.InfAmmoBackpackAdded = lplr.Backpack.ChildAdded:Connect(function(child)
                if child:IsA("Tool") and Script.Temp.InfAmmoEnabled then
                    task.wait(0.2)
                    setInfAmmo(true)
                end
            ...
            local count = setInfAmmo(false)
            Script.Functions.Alert("Infinite Ammo Disabled: " .. count .. " restored", 3)
            
            Script.Temp.InfAmmoEnabled = false
            
            if Script.Temp.InfAmmoLoop then
                task.cancel(Script.Temp.InfAmmoLoop)
                Script.Temp.InfAmmoLoop = nil
            end
            
            if Script.Temp.InfAmmoToolAdded then
                Script.Temp.InfAmmoToolAdded:Disconnect()
                Script.Temp.InfAmmoToolAdded = nil
            end
            
            if Script.Temp.InfAmmoBackpackAdded then
                Script.Temp.InfAmmoBackpackAdded:Disconnect()
                Script.Temp.InfAmmoBackpackAdded = nil
            end
    ...
    Options.CustomFireRate:OnChanged(function(val)
        if Toggles.CustomRapidFire.Value then
            setFireRate(true, val)
            Script.Functions.Alert("FireRate updated: " .. val, 2)
        end
    end)
    
    Options.CustomBulletsPerFire:OnChanged(function(val)
        if Toggles.CustomMultiShot.Value then
            setBulletsPerFire(true, val)
            Script.Functions.Alert("BulletsPerFire updated: " .. val, 2)
        end
    end)
    
    Options.CustomSpreadValue:OnChanged(function(val)
        if Toggles.CustomSpread.Value then
            setSpread(true, val)
            Script.Functions.Alert("Spread updated: " .. val, 2)
        end
    end)
    
    Options.CustomRecoilValue:OnChanged(function(val)
        if Toggles.CustomRecoil.Value then
            setRecoil(true, val)
            Script.Functions.Alert("Recoil updated: " .. val, 2)
        end
    end)
end
...
function Script.Functions.SpoofFlingVelocity(call)
    if call then
        ...
        Script.Functions.Alert("UNWORK", 3)
    else
        --[[
...
function Script.Functions.FlingCharacterHook(call)
    if call then
        ...
        Script.Functions.Alert("Anticheat DETECTED!", 3)
    else
        --[[
        ...
        Script.Functions.Alert("Anticheat UnDisabled!", 3)
    end
end
...
local PlayerGroupBox = Tabs.Other:AddRightGroupbox("Player", "user") do
    PlayerGroupBox:AddSlider("SpeedSlider", {
        Text = "Walk Speed",
        Default = 80,
        Min = 0,
...
local PlayerAttachGroupBox = Tabs.Other:AddLeftGroupbox("Player Attach", "users-round") do
    PlayerAttachGroupBox:AddLabel("HIGH DETECTION RATE!")
...
CreditsGroup:AddLabel("skaneexx - Rework Scripter")
CreditsGroup:AddLabel("Rut1n - Help Rework Scripting")
CreditsGroup:AddLabel("linoria - UI Library")
CreditsGroup:AddLabel("ANGELWINGS TEAM")
